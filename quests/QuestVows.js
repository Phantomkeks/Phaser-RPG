...
class QuestVows extends BaseScene {
  ...
  create() {
    ...
    this.blanks = [
      { lead: 'I promise to ',
        choices: ['hold your hand',  'make you laugh', 'share my fries'] },
      { lead: ' when you are ',
        choices: ['scared',          'tired',           'grumpy'] },
      { lead: ', to ',
        choices: ['dance with you',  'cook for you',    'listen closely'] },
      { lead: ' even on ',
        choices: ['rainy days',      'long Mondays',    'forgotten birthdays'] },
      { lead: ', and to always ',
        choices: ['choose us',       'come back home',  'love you bigger'],
        tail: '.' }
    ];
    ...
    this.scrollText = this.add.text(width / 2, 130, '', {
      ...THEME.text.body, color: THEME.colors.white,
      align: 'center', wordWrap: { width: width - 100 }, lineSpacing: 6
    }).setOrigin(0.5, 0);
  ...
  renderStep() {
    ...
    step.choices.forEach((choice, i) => {
      const t = this.addButton(this.scale.width / 2, 320 + i * 50, `> ${choice}`,
        () => {
        ...
        { ...THEME.text.button, fontSize: '11px', padding: { x: 12, y: 8 },
  ...
  win() {
    ...
    this.tweens.add({
      targets: this.scrollText, alpha: { from: 1, to: 0.7 }, yoyo: true,
      duration: 600, repeat: 2
    });

    this.add.text(this.scale.width / 2, this.scale.height - 70,
      '♥ Sealed with a kiss ♥', {
