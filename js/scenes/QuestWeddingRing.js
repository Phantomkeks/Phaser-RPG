...
class QuestWeddingRing extends BaseScene {
  ...
  create() {
    ...
    this.addSubtitle('The ring is hidden! Click objects to search.', 60);
    ...
    positions.forEach((pos, i) => {
      ...
      obj.on('pointerdown', () => {
        if (isRing) { this.found(obj); return; }
        // Disable a checked decoy so the player doesn't keep clicking the same wrong thing.
        obj.disableInteractive();
        this.tweens.add({
          targets: obj, scale: 5, duration: 100, yoyo: true,
          onComplete: () => obj.setAlpha(0.4)
        });
